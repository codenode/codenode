...
class SessionManager(webapp.RequestHandler):
    ...
    def get(self):
        ...
        #self.response.out.write(str(vars))
        self.response.out.write(str(session_key))




class CompleteName(webapp.RequestHandler):
    ...
    def get(self):
        ...
        logging.info('To match%s'%statement)
        ...
        try:
            ...
            completer = rlcompleter.Completer(old_globals)
            matches = completer.global_matches(statement)
            matches = map(str, matches)
            logging.info('Matches %s'% str(matches))
            self.response.out.write(str(matches))
        finally:
            pass

class CompleteAttr(webapp.RequestHandler):
    """handle tab completion requests
    """
    ...
    def get(self):
        ...
        try:
            ...
            completer = rlcompleter.Completer(old_globals)
            matches = completer.attr_matches(statement)
...
class Evaluate(webapp.RequestHandler):
...
def main():
    application = webapp.WSGIApplication(
            ...
            ('/complete_attr', CompleteAttr),
            ('/evaluate', Evaluate),], debug=_DEBUG)
